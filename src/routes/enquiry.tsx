...
function StandardCard({ title, subtitle, icon: Icon, badge, badgeType = "default", children }: CardProps) {
  const badgeClasses = {
    success: "text-emerald-700 bg-emerald-50 border-emerald-250",
    warning: "text-amber-700 bg-amber-50 border-amber-250",
    info: "text-indigo-700 bg-indigo-50 border-indigo-250",
    default: "text-purple-700 bg-purple-50 border-purple-250"
  };

  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm hover:shadow-md hover:scale-[1.01] transition-all flex flex-col justify-between h-full group">
      <div>
        <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-100">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-slate-50 rounded-xl border border-slate-100 group-hover:bg-purple-50 group-hover:border-purple-100 transition-colors">
              <Icon className="w-5 h-5 text-purple-600" />
            </div>
            <div>
              <h3 className="text-base font-bold text-slate-900 font-serif leading-tight">{title}</h3>
              {subtitle && <p className="text-[10px] text-slate-500 font-medium font-sans mt-0.5">{subtitle}</p>}
            </div>
          </div>
          {badge && (
        ...
        <div className="mt-3 text-slate-600 font-sans text-sm leading-relaxed">
          {children}
        </div>
      </div>
    </div>
...
export default function EnquiryPage() {
  ...
  const [tier, setTier] = useState("General Advisory");
  ...
  useEffect(() => {
    document.title = "Secure Enquiry Portal — Zug Switzerland";
  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-[#FAF9FB] text-slate-900 font-sans pb-24 bg-grid-pattern">
      {/* Background Glowing Ambient Orbs */}
      <div className="absolute top-[5%] left-[10%] w-[600px] h-[600px] bg-purple-100/40 rounded-full blur-[130px] pointer-events-none z-0" />
      <div className="absolute bottom-[10%] right-[10%] w-[550px] h-[550px] bg-blue-50/50 rounded-full blur-[140px] pointer-events-none z-0" />
      ...
      <div className="relative z-20 max-w-7xl mx-auto px-6 py-12 lg:py-16 flex flex-col min-h-screen justify-between gap-12">
        ...
        <header className="flex justify-between items-center border-b border-slate-200 pb-6">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="font-serif text-2xl font-bold tracking-tight text-slate-900 flex items-baseline gap-2">
                Secure Registry <span className="text-[10px] uppercase tracking-wider text-white font-sans px-2 py-0.5 bg-purple-600 rounded-md font-bold">PRO</span>
              </span>
              <span className="text-[10px] text-slate-500 font-bold tracking-widest uppercase">ENQUIRY REGISTRY PORTAL</span>
            </div>
          </div>
          <button 
            onClick={handleBack} 
            className="text-xs font-bold text-purple-600 hover:text-purple-800 px-5 py-2.5 rounded-xl border border-purple-200 bg-white hover:bg-purple-50 shadow-sm transition-all cursor-pointer hover:shadow"
          >
            ← Back to Article
          </button>
        </header>
        ...
        <div className="text-center max-w-3xl mx-auto">
          <div className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-purple-50 border border-purple-200 text-purple-700 text-xs font-bold mb-4 shadow-sm">
            <Sparkles className="w-4 h-4 text-purple-600" />
            PRIVATE ROUND ADVISORY
          </div>
          <h1 className="font-serif text-4xl md:text-6xl font-bold tracking-tight text-slate-900 mb-6 leading-tight">
            Private Advisory Request
          </h1>
          <p className="text-slate-600 text-lg leading-relaxed font-sans">
            Submit your private allocation inquiry. Your contact profile is processed server-side through a dedicated proxy API and synchronized securely into our CRM pipeline.
          </p>
        </div>

        {/* -------------------- 1. CRYPTO TICKER MARQUEE LINE -------------------- */}
        <div className="w-full overflow-hidden border border-slate-200 bg-white/80 py-3.5 relative rounded-2xl shadow-sm z-10">
          <div className="anim-marquee flex gap-12 whitespace-nowrap text-xs font-mono font-bold text-slate-700">
            <span>• BTC/USD: $71,842.30 <span className="text-emerald-600 font-semibold">(+2.41%)</span></span>
            <span>• ETH/USD: $3,824.10 <span className="text-emerald-600 font-semibold">(+1.87%)</span></span>
            <span>• SOL/USD: $184.22 <span className="text-emerald-600 font-semibold">(+4.18%)</span></span>
            <span>• SWISS HYDRO VALIDATOR FEE: <span className="text-purple-600">12 GWEI</span></span>
            <span>• ACTIVE CHAIN NODES: <span className="text-emerald-600">849,203 ONLINE</span></span>
            <span>• BLOCKS IN MEMPOOL: <span className="text-indigo-600">1,421 SYNCED</span></span>
            <span>• SWISS SECURE CUSTODY PROTOCOL: <span className="text-emerald-600">ACTIVE</span></span>
            <span>• BTC/USD: $71,842.30 <span className="text-emerald-600 font-semibold">(+2.41%)</span></span>
            <span>• ETH/USD: $3,824.10 <span className="text-emerald-600 font-semibold">(+1.87%)</span></span>
            <span>• SOL/USD: $184.22 <span className="text-emerald-600 font-semibold">(+4.18%)</span></span>
            <span>• SWISS HYDRO VALIDATOR FEE: <span className="text-purple-600">12 GWEI</span></span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch">
          ...
          <div className="lg:col-span-2 bg-white border border-slate-200 rounded-3xl p-8 md:p-10 shadow-sm flex flex-col justify-between">
            <div>
              <h2 className="text-2xl font-bold text-slate-900 mb-8 flex items-center gap-3 font-serif border-b border-slate-100 pb-5">
                <FileText className="w-6 h-6 text-purple-600" />
                Sovereign Enquiry Form
              </h2>
              ...
              {submitSuccess ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-xs font-bold text-slate-700 mb-2.5 uppercase tracking-wider font-sans">Full Name</label>
                      <div className="relative">
                        ...
                        <input 
                          ...
                          placeholder="Nils Suter" 
                          className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-12 pr-5 py-4 text-[16px] text-slate-900 placeholder-slate-400 focus:bg-white focus:border-purple-500 focus:ring-4 focus:ring-purple-500/5 outline-none transition-all"
                        />
                      </div>
                    </div>
                    
                    <div>
                      <label className="block text-xs font-bold text-slate-700 mb-2.5 uppercase tracking-wider font-sans">Email Address</label>
                      <div className="relative">
                        <span className="absolute inset-y-0 left-0 pl-4.5 flex items-center text-slate-400">
                        ...
                        <input 
                          ...
                          placeholder="sovereign@partner.ch" 
                          className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-12 pr-5 py-4 text-[16px] text-slate-900 placeholder-slate-400 focus:bg-white focus:border-purple-500 focus:ring-4 focus:ring-purple-500/5 outline-none transition-all"
                        />
                      </div>
                    </div>
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-2.5 uppercase tracking-wider font-sans">Phone / Number</label>
                    <div className="relative">
                      <span className="absolute inset-y-0 left-0 pl-4.5 flex items-center text-slate-400">
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-2.5">
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider font-sans">Verification / Message Details</label>
                      <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wide">Optional</span>
                    </div>
                    <div className="relative">
                      <span className="absolute top-4 left-4.5 text-slate-400">
                      ...
                      <textarea 
                        ...
                        placeholder="State your allocation target, tax queries, or cold storage preferences (optional)..." 
                        className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-12 pr-5 py-4 text-[16px] text-slate-900 placeholder-slate-400 focus:bg-white focus:border-purple-500 focus:ring-4 focus:ring-purple-500/5 outline-none transition-all font-sans"
                      />
                    </div>
                  </div>
                  ...
                  {isSubmitting ? (
                    <button 
                      type="button"
                      disabled
                      className="w-full flex items-center justify-center gap-3 bg-purple-900 border border-purple-800 py-4.5 rounded-xl font-bold text-base uppercase tracking-wider text-purple-100 shadow"
                    >
                      <RefreshCw className="w-5 h-5 animate-spin" />
                      Syncing to CRM Secure Database...
                    </button>
                  ) : (
                    <button 
                      type="submit"
                      className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-purple-600 via-indigo-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 border border-purple-500/20 py-4.5 rounded-xl font-bold text-base uppercase tracking-wider text-white shadow-lg shadow-purple-100 transition-all active:scale-[0.99] group cursor-pointer"
                    >
                      Transmit Enquiry to CRM
          ...
          <div className="flex flex-col gap-8 justify-between">
            ...
            <StandardCard
              ...
              badgeType="success"
            >
              <div className="bg-slate-50 border border-slate-100 rounded-xl p-4 flex flex-col justify-between h-28 relative overflow-hidden mb-2 mt-1">
                <svg viewBox="0 0 300 120" className="w-full h-20 overflow-visible">
                  {/* Grid Lines */}
                  <line x1="0" y1="100" x2="300" y2="100" stroke="rgba(30, 27, 75, 0.05)" strokeWidth="1" />
                  <line x1="0" y1="70" x2="300" y2="70" stroke="rgba(30, 27, 75, 0.05)" strokeWidth="1" />
                  <line x1="0" y1="40" x2="300" y2="40" stroke="rgba(30, 27, 75, 0.05)" strokeWidth="1" />
                  ...
                  <path 
                    d="M 10 90 Q 70 85 110 55 T 220 35 T 290 15" 
                    fill="none" 
                    stroke="url(#gradient-purple-blue-2)" 
                    strokeWidth="3.5" 
                    className="anim-chart-line"
                  />
                  {/* Target Point */}
                  <circle cx="290" cy="15" r="4" fill="#6D28D9" />

                  {/* Gradient Definitions */}
                  <defs>
                    <linearGradient id="gradient-purple-blue-2" x1="0%" y1="0%" x2="100%" y2="100%">
                ...
                <div className="flex justify-between items-center text-xs font-mono text-slate-400 mt-2">
                  <span>YTD Performance: <strong className="text-emerald-600 font-bold">+58.4%</strong></span>
                  <span className="text-purple-600 font-bold">CHF 1.4B CAP</span>
                </div>
              </div>
            </StandardCard>
            ...
            >
              <div className="grid grid-cols-2 gap-3 mt-1.5">
                <div className="bg-slate-50 border border-slate-100 p-3 rounded-xl">
                  <div className="text-xs text-slate-500 uppercase font-bold font-sans tracking-wide">Gas Price</div>
                  <div className="text-base font-bold text-slate-800 font-mono mt-0.5">12 Gwei</div>
                </div>
                <div className="bg-slate-50 border border-slate-100 p-3 rounded-xl">
                  <div className="text-xs text-slate-500 uppercase font-bold font-sans tracking-wide">Validation</div>
                  <div className="text-base font-bold text-emerald-600 font-mono mt-0.5">99.98%</div>
                </div>
                <div className="bg-slate-50 border border-slate-100 p-3 rounded-xl">
                  <div className="text-xs text-slate-500 uppercase font-bold font-sans tracking-wide">Block Time</div>
                  <div className="text-base font-bold text-slate-800 font-mono mt-0.5">1.2s avg</div>
                </div>
                <div className="bg-slate-50 border border-slate-100 p-3 rounded-xl">
                  <div className="text-xs text-slate-500 uppercase font-bold font-sans tracking-wide">Staking Pool</div>
                  <div className="text-base font-bold text-purple-600 font-mono mt-0.5">Active</div>
                </div>
              </div>
            </StandardCard>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-4 font-mono text-xs text-slate-500">
              <div>
                <div className="flex justify-between mb-1.5 font-bold">
                  ...
                  <span className="text-emerald-650">92% Filled</span>
                </div>
              ...
              <div>
                <div className="flex justify-between mb-1.5 font-bold">
                  ...
                  <span className="text-purple-650">76% Filled</span>
                </div>
                <div className="h-2.5 bg-slate-100 rounded-full overflow-hidden border border-slate-200 shadow-inner">
                  <div 
          ...
          <StandardCard
            ...
            badge="Ledger Sync"
          >
            <p className="text-sm text-slate-550 mb-3 leading-relaxed">
            ...
            <div className="grid grid-cols-2 gap-3 font-mono text-xs">
              <div className="border border-slate-200 bg-slate-50 p-2.5 rounded-xl">
                <div className="text-slate-400 text-[10px] font-bold uppercase">Block height</div>
                <div className="text-purple-600 font-bold text-sm mt-0.5">#849,203</div>
                <div className="text-[10px] text-slate-500 mt-0.5">12 Txns • Validated</div>
              </div>
              <div className="border border-slate-200 bg-slate-50 p-2.5 rounded-xl">
                <div className="text-slate-400 text-[10px] font-bold uppercase">Block height</div>
                <div className="text-purple-600 font-bold text-sm mt-0.5">#849,204</div>
                <div className="text-[10px] text-emerald-600 font-bold mt-0.5">Processing Nodes</div>
              </div>
            </div>
          </StandardCard>
          ...
          <StandardCard
            ...
            badgeType="info"
          >
            <p className="text-sm text-slate-550 mb-3.5 leading-relaxed">
            ...
            <div className="h-20 relative border border-slate-200 rounded-xl bg-slate-50 flex items-center justify-center overflow-hidden">
              <div className="absolute left-[15%] top-[45%] flex items-center gap-1.5">
                ...
                <span className="text-[11px] font-bold font-mono text-slate-500">Geneva</span>
              </div>
              <div className="absolute left-[45%] top-[25%] flex items-center gap-1.5">
                ...
                <span className="text-[11px] font-bold font-mono text-slate-500">Zurich</span>
              </div>
              <div className="absolute left-[65%] top-[50%] flex items-center gap-1.5">
                ...
                <span className="text-[11px] font-bold font-mono text-purple-700">Zug</span>
              </div>
              <div className="absolute left-[55%] top-[72%] flex items-center gap-1.5">
                ...
                <span className="text-[11px] font-bold font-mono text-slate-500">Lugano</span>
              </div>
            </div>
          </StandardCard>
        ...
        <footer className="border-t border-slate-200 pt-8 flex flex-col sm:flex-row justify-between items-center gap-4 text-xs text-slate-500 font-semibold">
          <div>© 2026 Private Enquiry Registry. All rights reserved.</div>
          <div className="flex gap-6">
            <a 
              ...
              className="hover:text-purple-700 font-bold transition-colors"
            >
              Privacy Policy
            </a>
            <a 
